...
def render() -> None:
    """Render the Volatility Brain page.

    Engle (2003 Nobel) — GJR-GARCH captures time-varying volatility and
    leverage effects, the key driver of tail-risk underestimation.
    """
    ...
    symbol = col_sym.text_input(
        "Ticker Symbol", value="SPY", placeholder="SPY, AAPL…",
        key="_vb_ticker",
    ).upper().strip()
    years = col_yrs.number_input(
        "Lookback (yrs)", min_value=2, max_value=10, value=5, key="_vb_years_input",
    )
    run = col_btn.button("▶ Run GARCH", type="primary", key="_vb_run")

    _cached_sym = st.session_state.get("_vb_symbol", "")
    _cached_yrs = st.session_state.get("_vb_years", -1)
    _stale      = (symbol != _cached_sym or years != _cached_yrs)

    if run or ("_vb_result" in st.session_state and not _stale):
        if run or _stale:
            with st.spinner(f"Fitting GJR-GARCH on {symbol}…"):
                try:
                    g = _run_garch(symbol, years)
                    st.session_state["_vb_result"] = g
                    st.session_state["_vb_symbol"] = symbol
                    st.session_state["_vb_years"]  = years
                except Exception as exc:
                    st.error(f"GARCH failed: {exc}")
                    return
        else:
            g = st.session_state["_vb_result"]
        ...
        with col_left:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge={
                    "axis": {"range": [0.80, 1.0], "tickcolor": "#E0E0E0",
                             "tickfont": {"size": 10}},
        ...
        with st.expander("🏦 Merton Distance-to-Default (optional)"):
            ...
            rfr        = m_col2.number_input("Risk-Free Rate", min_value=0.0, max_value=0.2,
                                              ...
                                              key="_vb_rfr")
            run_merton = m_col3.button("Compute D2D", key="_vb_merton")

            if run_merton:
                with st.spinner("Running Merton model…"):
                    try:
                        mrt = _run_merton(symbol, face_debt, rfr)
                        d2d_icon = ("🔴" if mrt["d2d"] < 1.5
                                    else ("🟡" if mrt["d2d"] < 3.0 else "🟢"))
                        mc1, mc2, mc3 = st.columns(3)
                        mc1.metric("Distance-to-Default", f"{mrt['d2d']:.3f} σ",
                                   delta=d2d_icon)
