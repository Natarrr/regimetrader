...
st.set_page_config(page_title="Valuation Radar", page_icon="📈", layout="wide")
...
# Historical CAPE danger thresholds
_CAPE_HIGH   = 30.0   # historically elevated
_CAPE_BUBBLE = 40.0   # dot-com / 2021-era territory
...
with col_chart:
    ...
    # Keep last 40 years for clarity
    ...
    fig_cape.add_hrect(y0=_CAPE_BUBBLE, y1=series_40.max() * 1.1,
                       ...
                       annotation_text="Bubble Territory", annotation_font_color=_RED,
                       annotation_position="top left")
    fig_cape.add_hrect(y0=_CAPE_HIGH, y1=_CAPE_BUBBLE,
                       fillcolor="rgba(255,215,0,0.05)", line_width=0,
                       annotation_text="Elevated", annotation_font_color=_GOLD,
                       annotation_position="top left")
    ...
    # Annotate 2000 & 2008 crashes — use add_shape + add_annotation separately
    # (add_vline with annotation calls sum([str, str]) internally which breaks on date strings)
...
with col_gauge:
    ...
    fig_ecy = go.Figure(go.Bar(
        ...
        text=[f"{v:.2f}%" for v in [100/d["current_cape"], d["real_10y"]*100, ecy_val]],
