...
from backend.quant_models.monetary_pulse import yield_spread, is_inverted, monetary_regime, m2_velocity_trend
...
st.set_page_config(page_title="Regime Prediction",
...
def _run_hmm(sym: str) -> dict:
    ...
    return {
        ...
        "regime_probs":   [float(p) for p in state.regime_probs] if state.regime_probs is not None else [],
        "label_sequence": label_sequence,
        "color_map":      dict(state.color_map),
    }


# ── Ticker map: display name → Yahoo Finance symbol ───────────────────────────
...
_default = _TICKER_MAP.get(
    st.session_state.get("_qs_symbol", "SPY"), "SPY"
)
...
# Resolve display name → Yahoo Finance symbol
...
# ── Run analysis ───────────────────────────────────────────────────────────────
if run or "regime_result" in st.session_state:
    if run:
        ...
        with st.spinner("Fetching all data sources…"):
            try:
                ...
                macro = {"spread_latest": 0.0, "inverted": False,
                         "mon_regime": "NEUTRAL", "m2v_trend": "STABLE",
                         "spread": None}
            ...
            try:
                ...
                hmm = {"hmm_label": "Unknown", "position_scale": 1.0,
                       "is_uncertain": False, "regime_probs": None,
                       "label_sequence": []}
                st.warning(f"HMM partial: {exc}")

        laureate = classify_regime(
            hmm["hmm_label"], macro["mon_regime"], garch["vol_regime"]
        )
        ...
        final_scale = combined_position_scale(
            hmm.get("position_scale", 1.0), laureate)

        st.session_state["regime_result"] = dict(
            symbol=symbol, display_name=_display_name, macro=macro, cape_d=cape_d,
            garch=garch, hmm=hmm, laureate=laureate, minsky=minsky,
            final_scale=final_scale,
        )
        # Clear progress banners
        col_p1.empty()
        col_p2.empty()
        col_p3.empty()
    ...
    final_scale = r.get("final_scale", combined_position_scale(
        hmm.get("position_scale", 1.0), laureate))

    # ── Laureate Regime banner ─────────────────────────────────────────────────
    ...
    minsky_icons = {"CRITICAL": "🚨",
                    "WARNING": "⚠️", "WATCH": "👁️", "CLEAR": "✅"}
    ...
    c1.metric("HMM Label", hmm.get("hmm_label", "Unknown"),
              delta=f"HMM scale {hmm.get('position_scale', 1.0):.2f}", delta_color="off")
    ...
    with col_m:
        ...
        for label, val, threshold, desc, triggered in cond_data:
        ...
        fig_conds = go.Figure(go.Indicator(
            ...
            title={"text": "Conditions Met", "font": {
        ...
        if label_seq:
            ...
            for lbl in unique_labels:
                ...
                fig_hist.add_trace(go.Scatter(
                    ...
                    name=lbl,
                    marker=dict(
                        color=_LABEL_COLORS.get(lbl, _PURPLE),
            ...
            fig_hist.update_layout(
                ...
                yaxis=dict(
                    tickvals=list(range(len(set(label_seq)))),
            ...
            if probs is not None and hasattr(probs, "__len__"):
                fig_probs = go.Figure(go.Bar(
                    ...
                    y=list(probs),
                    marker_color=[_GREEN, _GOLD, _RED,
                                  _PURPLE, _BLUE][:len(probs)],
    ...
    st.info("Enter a ticker and click **▶ Analyze** to compute the composite regime and Minsky alert.")
    st.markdown("""
...
with st.expander("Model notes"):
    st.markdown("""
### 🧭 Regime Prediction – Quick Guide

#### 🔹 System Logic
The dashboard identifies the **market regime** (Stable, Fragile, or Crisis) using several sensors.
Each sensor measures a different risk dimension: volatility, valuation, liquidity, or macro tension.
The goal is to help users **take clear actions** based on market signals.

---

### ⚙️ Sensors and Actions

| Sensor | What it means | Suggested Action |
|--------|----------------|------------------|
| **GARCH Persistence** | Measures lasting volatility. | If > threshold → reduce exposure, increase hedging. |
| **CAPE Percentile** | Shows how expensive equities are. | If > 95th → avoid new entries, expect correction. |
| **Yield Spread** | Reflects economic health via yield curve. | If narrow/inverted → prepare for slowdown, shift to defensive assets. |
| **Policy Scale** | Indicates monetary policy intensity. | If low → liquidity tightening, reduce leverage. |
| **Volatility Brain** | Tracks volatility structure changes. | If unstable → avoid directional trades. |
| **Monetary Pulse** | Measures money flow and central bank rhythm. | If slowing → reduce risk exposure. |
| **Valuation Radar** | Detects asset overvaluation. | If extreme → favor cash or undervalued sectors. |
| **Contagion Web** | Maps risk connections across sectors. | If high → diversify, reduce correlated positions. |

---
### ⚙️ Regime probalities and interpretation 
            
State 0 — Stable / Low Volatility (Positive Drift)  
Calm markets, low volatility, slow positive trend. “Normal expansion.”

State 1 — Slow Decline / Low Volatility (Negative Drift)  
Market is calm but drifting down. Early deterioration phase.

State 2 — Fragile / High Volatility (Negative Drift)  
Volatility elevated, returns weak or negative. Stress building.

State 3 — Crisis / Very High Volatility (Crash Dynamics)  
Sharp drops, volatility spikes, liquidity stress. Tail‑risk regime.

State 4 — Recovery / High Volatility (Positive Drift)  
Rebound phase after crisis. Volatile but trending upward.

### 🧩 Regime Reading
- **STABLE** → Balanced market, normal conditions.  
- **FRAGILE** → High volatility, stretched valuations → caution.  
- **CRISIS / BEAR** → Systemic risk → cut positions, raise liquidity.

---

### 💡 Practical Tips
- Watch **trigger levels**: they mark critical zones.  
- Combine signals to understand the **global regime**.  
- **Minsky WATCH** alerts mean rising instability risk.
