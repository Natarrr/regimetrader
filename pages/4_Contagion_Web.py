...
def render() -> None:
    """Render the Contagion Web page.

    Leontief (1973 Nobel) — I-O model captures all indirect supply-chain
    dependencies; high-multiplier sectors amplify any demand shock.
    """
    ...
    sector    = col_s.selectbox("Shocked Sector", SECTORS,
                                index=SECTORS.index("Energy"), key="_cw_sector")
    shock_pct = col_p.slider("Demand Shock (%)", min_value=-100, max_value=-1,
                              value=-20, step=1, key="_cw_shock")
    run       = col_b.button("▶ Simulate", type="primary", key="_cw_run")

    if run or "_cw_contagion_result" in st.session_state:
        if run:
            impacts = shock_propagation(A, {sector: shock_pct / 100.0})
            gdp_imp = total_gdp_impact(impacts)
            st.session_state["_cw_contagion_result"] = (sector, shock_pct,
                                                         impacts, gdp_imp)
        else:
            sector, shock_pct, impacts, gdp_imp = st.session_state["_cw_contagion_result"]
        ...
        c1.metric("GDP Impact (weighted)", f"{gdp_imp:.2f}%", delta_color="inverse")
        c2.metric(f"{sector} Output Impact", f"{shocked_impact:.2f}%", delta_color="inverse")
    ...
    mult_df_items = sorted(zip(SECTORS, multipliers.tolist()), key=lambda x: -x[1])
